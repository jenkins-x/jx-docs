...
One nice benefit of using `helm template` to generate the YAML then using `kubectl apply` means that we can look at optionally using tools like [kustomize](--remote-tiller=) post process the output of `helm template` to allow resources to be overridden or enriched in times that the chart author did not think of.
