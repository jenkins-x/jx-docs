...
If Jenkins X was installed in the namespace `jx` then the following should [switch your context](/docs/using-jx/tasks/kube-context/) to the `jx` namespace:
...
By default when you [install Jenkins X into an existing kubernetes cluster](/docs/managing-jx/tasks/install-on-cluster/) it prompts you if you want to install an Ingress controller. Jenkins X needs an Ingress controller of some kind so that we can setup `Ingress` resources for each `Service` so we can access web applications via URLs outside of the kubneretes cluster (e.g. inside web browsers).
