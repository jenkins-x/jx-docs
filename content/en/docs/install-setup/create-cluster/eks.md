...
You need the id and secret for running [`jx boot`](/docs/install-setup/create-cluster/eks/#running-jx-boot).
...
Refer to [Production cluster considerations](/docs/install-setup/create-cluster/eks/#production-cluster-considerations) for things to consider when creating a production cluster.
{{% /alert %}}
...
You can choose to create S3 buckets for [long term storage](/docs/install-setup/boot/storage/) with `enable_logs_storage`, `enable_reports_storage` and `enable_repository_storage`.
...
{{% alert title="Warning" color="warning" %}}
If you set `force_destroy` to false, and run a `terraform destroy`, it will fail. In that case empty the s3 buckets from the aws s3 console, and re run `terraform destroy`.
{{% /alert %}}
...
You can also configure an existing Vault instance for the use with Jenkins X.
In this case provide the Vault URL via the _vault_url_ via the input variable and follow the Jenkins X documentation around the instllation of an [external Vault](https://jenkins-x.io/docs/install-setup/installing/boot/secrets/#external) instance.
...
This _jx_requirements_ output can be used as input to [Jenkins X Boot](/docs/getting-started/setup/boot/) which is responsible for installing all the required Jenkins X components into the cluster created by this module.
