...
title: Features
linktitle: Features
description: How JayeX can help you deliver continuously
weight: 30
---
...
JayeX comes with a handy [jx](/commands/jx/) command line tool to easily:

* [install JayeX](/docs/install-setup/boot/) inside your existing kubernetes cluster
* [create a new kubernetes cluster](/docs/getting-started/setup/create-cluster/) and install JayeX into it
* [import projects](/docs/resources/guides/using-jx/creating/import/) into JayeX and their Continuous Delivery pipelines setup
* [create new Spring Boot applications](/developing/create-spring/) which get imported into JayeX and their Continuous Delivery pipelines setup

## Automated Pipelines

JayeX will automatically set up awesome pipelines for your projects that fully implement both CI and CD using [DevOps best practices](/about/concepts/).
...
With JayeX each _team_ gets its own Environments. By default JayeX creates a `Staging` and `Production` environment for each team but you can create new environments via [jx create environment](/commands/jx_create_environment/).
...
A Team in JayeX is represented by an install of JayeX in a separate namespace.

You can install JayeX into different namespaces in the same cluster if you wish using the `--namespace` command line argument in [jx create cluster](/commands/jx_create_cluster/) or [jx install](/commands/deprecation/). Note that to support multiple installs of JayeX in the same cluster you need to [avoid Tiller if you are using helm 2.x](/news/helm-without-tiller/).

You can also use the [jx create team](/commands/jx_create_team/) CLI which creates a new `Team` [Custom Resource](/docs/reference/components/custom-resources/) then in the background the team controller will create a new JayeX install in the teams namespaces, by default reusing the same underlying nexus and docker registry.
...
The CD Pipelines of JayeX automate the promotion of version changes through each Environment which is configured with a _promotion strategy_ property of `Auto`. By default the `Staging` environment uses automatic promotion and the `Production` environment uses `Manual` promotion.
...
As you can see above JayeX automatically comments on your Pull Requests when using Preview Environments.

If the commit comments reference issues (e.g. via the text `fixes #123`) then JayeX pipelines will generate release notes like those of [the jx releases](https://github.com/jenkins-x/jx/releases).
...
A collection of best of breed software tools packaged as helm charts that come pre-integrated with JayeX such as: Nexus, ChartMuseum, Monocular, Prometheus, Grafana etc
